...
codeunit 148147 "PEPPOL BIS 3.0 XML Tests"
{
    ...
    [Test]
    procedure ExportSalesInvIncludesLongRegulatoryCommentAsSingleNote()
    var
        SalesCommentLine: Record "Sales Comment Line";
        SalesHeader: Record "Sales Header";
        SalesInvoiceHeader: Record "Sales Invoice Header";
        ...
        InvoiceNo: Code[20];
    begin
        // [SCENARIO] A mandatory PMD comment longer than 80 characters is exported as one tagged UBL note
        Initialize();

        CustomerNo := CreateCustomer('', "Electronic Address Scheme"::"EM");
        InvoiceNo := CreateSalesInvoiceWithLine(CustomerNo);
        RegulatoryComment := 'Tout retard de paiement engendre une pénalité exigible à compter de la date d''échéance, calculée sur la base de trois fois le taux d''intérêt légal.';
        SalesCommentLine."Document Type" := SalesCommentLine."Document Type"::Invoice;
        SalesCommentLine."No." := InvoiceNo;
        SalesCommentLine."Line No." := 10000;
        SalesCommentLine."FR Regulatory Comment Type" := SalesCommentLine."FR Regulatory Comment Type"::PMD;
        SalesCommentLine."FR Regulatory Comment" := CopyStr(RegulatoryComment, 1, MaxStrLen(SalesCommentLine."FR Regulatory Comment"));
        SalesCommentLine.Insert();
        SalesHeader.Get("Sales Document Type"::Invoice, InvoiceNo);
        SalesInvoiceHeader.Get(LibrarySales.PostSalesDocument(SalesHeader, true, true));
        ...
        Assert.AreEqual('#PMD#' + RegulatoryComment,
